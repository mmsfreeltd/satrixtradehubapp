...
export const client_expert = pgTable("client_expert", {
  ...
  payment_status: integer("payment_status").default(0),
});
...
export const loans = pgTable("loans", {
  id: serial("id").primaryKey(),
  user_id: integer("user_id").notNull(),
  amount_requested: doublePrecision("amount_requested").notNull(),
  amount_approved: doublePrecision("amount_approved"),
  status: varchar("status", { length: 20 }).notNull().default("pending"), // pending|approved|rejected
  date_requested: timestamp("date_requested").notNull().defaultNow(),
...
export const user_documents = pgTable("user_documents", {
  id: serial("id").primaryKey(),
  user_id: integer("user_id").notNull(),
  doc_type: varchar("doc_type", { length: 200 }).notNull(),
  file_url: varchar("file_url", { length: 2000 }).notNull(),
  public_id: varchar("public_id", { length: 500 }).notNull(),
  date_uploaded: timestamp("date_uploaded").defaultNow().notNull(),
});
...
export const simulations = pgTable("simulations", {
  ...
  starting_balance: doublePrecision("start_bal").notNull(),
  current_balance: doublePrecision("curr_bal").notNull(),
  date_created: timestamp("date_created").defaultNow(),
});

export const simulation_trades = pgTable("simulation_trades", {
  id: serial("id").primaryKey(),
  simulation_id: integer("simulation_id")
    .references(() => simulations.id)
    .notNull(),
  coin_id: varchar("coin_id", { length: 50 }).notNull(),
  type: varchar("type", { length: 10 }).$type<"Buy" | "Sell">().notNull(),
  amount: doublePrecision("amount").notNull(),
  entry_price: doublePrecision("entry_price").notNull(),
  exit_price: doublePrecision("exit_price"),
  profit_loss: doublePrecision("pnl"),
